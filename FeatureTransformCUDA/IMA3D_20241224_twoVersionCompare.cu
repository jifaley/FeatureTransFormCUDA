...
template <typename T>
int compareArraysIfSameSign(const T* arr1, const T* arr2, size_t size) {
	int count = 0;
...
void convertNewfts2old(int* oldfts, int* newfts, int xdim, int ydim, int zdim) {
	for (int i = 0; i < xdim * ydim * zdim; ++i) 
	{
		int curftIdx = oldfts[i];
		int z = curftIdx / (ydim * xdim);
		int y = curftIdx % (ydim * xdim) / xdim;
		int x = curftIdx % xdim;
		int newftIdx = x * ydim * zdim + y * zdim + z;
		newfts[i] = newftIdx;
	}
}
/*************** MAIN PROGRAM *****************/
...
int main(int argc, const char **argv)
{
	...
	//dealWithInput(h_inputImagePtr_new, allsize, max);
	...
	convertNewfts2old(ft_test->data, ft_test->data, xdim, ydim, zdim);

	int count25 = compareArrays(ft_test->data, ft->data, allsize);
	cout << "count2.5: " << count25 << endl;
	...
	runSkeletonization(ft_test, indata_test, gamma_val);

	std::cerr << ft->xdim << " " << ft->ydim << " " << ft->zdim << std::endl;
	std::cerr << ft_test->xdim << " " << ft_test->ydim << " " << ft_test->zdim << std::endl;


	int count3 = compareArrays(indata->data, indata_test->data, allsize);
	cout << "count3: " << count3 << endl;

	int count3Sign = compareArraysIfSameSign(indata->data, indata_test->data, allsize);
	cout << "count3Sign: " << count3Sign << endl;
	...
	int count4Sign = compareArraysIfSameSign(indata->data, indata_test->data, allsize);
	cout << "count4Sign: " << count4Sign << endl;

	std::cerr << "final cost " << timer.getTimerMilliSec() << "ms" << std::endl << std::endl;
	timer.update();

	
	saveTiff(output_file, indata_test->data, size);

	std::string output_file2 = "output_original.tif";

	saveTiff(output_file2.c_str(), indata->data, size);

	// Cleanup
	ByteVolume_Delete(indata);
	IntVolume_Delete(ft);
	ByteVolume_Delete(indata_test);
	IntVolume_Delete(ft_test);
