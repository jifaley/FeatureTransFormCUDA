...
Volume *Volume_New(int xdim, int ydim, int zdim)
{
	...
	if (!v->data) {
		printf("Out of memory: volume new!\n");
		exit(-1);
	}

	v->data[0] = (VOXEL **)calloc(xdim * ydim, sizeof(VOXEL *));
	if (!v->data[0]) {
		printf("Out of memory: volume new 2!\n");
		exit(-1);
	}

	v->data[0][0] = (VOXEL *)calloc(xdim * ydim * zdim, sizeof(VOXEL));
	if (!v->data[0][0]) {
		printf("Out of memory: volume new 3!\n");
		exit(-1);
	}
...
void ByteVolume_WriteVTK(ByteVolume* v, const char* filename)
{
	...
	for (int k = 0; k<v->zdim; k++)
		for (int j = 0; j<v->ydim; j++)
			...
			{
				unsigned char byteval = v->data[i + j * v->xdim + k * v->xdim * v->ydim];
...
ByteVolume *ByteVolume_New(int xdim, int ydim, int zdim)
{
	...
	v->data = (BYTE *)calloc(xdim * ydim * zdim, sizeof(BYTE));
	if (!v->data) {
		printf("Out of memory: bytevolume new!\n");
		exit(-1);
	}
	...
	if (ftype == 1) {
		for (k = 0; k<tzdim; k++)
			for (j = 0; j<tydim; j++)
				for (i = 0; i<txdim; i++) {
					fread(&byteval, sizeof(unsigned char), 1, f);
					temp->data[i + j * txdim + k * txdim * tydim] = byteval;
				}
	}
	else {
...
ByteVolume *ByteVolume_ReadVTK(char* filename)
{
	...
	for (k = 0; k<zdim; k++)
		for (j = 0; j<ydim; j++)
			...
			{
				fread(&byteval, sizeof(unsigned char), 1, f);
				temp->data[i + j * xdim + k * xdim * ydim] = byteval;
			}
...
void ByteVolume_MinMax(ByteVolume *v, BYTE *min, BYTE *max)
{
	...
	minval = maxval = v->data[0];

	for (z = 0; z<v->zdim; z++)
		for (y = 0; y<v->ydim; y++)
			for (x = 0; x<v->xdim; x++) {
				val = v->data[x + y * v->xdim + z * v->xdim * v->ydim];
...
void ByteVolume_Clamp(ByteVolume *v, BYTE clampval)
{
	...
	for (int i = 0; i < v->xdim * v->ydim * v->zdim; ++i) {
		v->data[i] = std::min(v->data[i], clampval);
	}
}
...
void ByteVolume_WriteSFF(ByteVolume *v, const char filename[])
{
	...
	for (k = 0; k<v->zdim; k++)
		for (j = 0; j<v->ydim; j++)
			for (i = 0; i<v->xdim; i++) {
				byteval = v->data[i + j * v->xdim + k * v->xdim * v->ydim];
				fwrite(&byteval, sizeof(unsigned char), 1, f);
			}
...
void ByteVolume_Clear(ByteVolume *v) {
	...
	for (x = 0; x<v->xdim; x++)
		for (y = 0; y<v->ydim; y++)
			for (z = 0; z<v->zdim; z++) {
				v->data[x + y * v->xdim + z * v->xdim * v->ydim] = 0;
			}
}
...
IntVolume *IntVolume_New(int xdim, int ydim, int zdim)
{
	...
	v->data = (int*)calloc(xdim * ydim * zdim, sizeof(int));
	if (!v->data) {
		printf("Out of memory: intvolume new!\n");
		exit(-1);
	}
